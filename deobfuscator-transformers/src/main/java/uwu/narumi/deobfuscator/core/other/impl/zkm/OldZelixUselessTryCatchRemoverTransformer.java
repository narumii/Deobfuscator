 ...
 * </pre>
 */
public class OldZelixUselessTryCatchRemoverTransformer extends Transformer {
  ...
  private static final Match INVOKED_AND_RETURN =
      SequenceMatch.of(
      ...
      classWrapper.methods().forEach(methodNode -> {
        ...
        methodNode.tryCatchBlocks.removeIf(tryBlock -> {
          ...
          if (result != null) {
            ...
            InsnContext start2 = methodContext.at(tryBlock.handler.getNext());
            MatchContext result2 = INVOKED_AND_RETURN.matchResult(start2);
            if (result2 != null) {
              MethodRef methodRef = MethodRef.of((MethodInsnNode) result2.insn());
