...
/**
 * Simplifies compare instructions on two constant values.
 */
public class CompareInstructionsTransformer extends FramedInstructionsTransformer {
  ...
  protected boolean transformInstruction(ClassWrapper classWrapper, MethodNode methodNode, AbstractInsnNode insn, Frame<OriginalSourceValue> frame) {
    // Get instructions from stack that are passed to compare instruction
    ...
    if (value1Insn.isNumber() && value2Insn.isNumber()) {
      ...
      methodNode.instructions.set(insn, AsmHelper.getNumber(AsmMathHelper.compare(value1, value2, insn.getOpcode())));
