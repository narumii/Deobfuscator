...
public class RakSzildStringTransformer extends Transformer {
            ...
            classNode.methods.forEach(methodNode -> Arrays.stream(methodNode.instructions.toArray())
                    ...
                    .forEach(node -> findMethod(classNode, method -> method.name.equals(node.name) && method.desc.equals(node.desc)).ifPresent(method -> {
                        getKey(method).ifPresent(key -> {
                            ...
                            String thirdMethod = "<clinit>"; //XD

                            methodNode.instructions.remove(node.getPrevious());
                            methodNode.instructions.set(node, new LdcInsnNode(decrypt(string, className, firstMethod, secondMethod, thirdMethod, key)));
                            toRemove.add(method);
                        });
                    })));
    ...
    private String decrypt(String string, String firstClass, String secondMethod, String firstMethod, String thirdMethod, int key) {
        ...
        for (int i = 0; i < chars.length; ++i) {
            chars[i] =
                    (char) (chars[i]
                            ^ firstClass.hashCode()
                            ^ secondMethod.hashCode()
                            ^ firstMethod.hashCode()
                            ^ thirdMethod.hashCode()
                            ^ key);
        }
